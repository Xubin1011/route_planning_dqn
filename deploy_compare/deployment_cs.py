...
def visualization(cs_path, p_path, route_path, source_lat, source_lon, target_lat, target_lon, map_name):
    ...
    bbox_coords = [[south_lat, west_lon], [north_lat, west_lon], [north_lat, east_lon], [south_lat, east_lon],
                   [south_lat, west_lon]]
    folium.Polygon(locations=bbox_coords, color='blue', fill=True, fill_color='blue', fill_opacity=0.2).add_to(
        map_object)
    ...
    path_infos = list(
        zip(path_data['Node'], path_data['Latitude'], path_data['Longitude'], path_data['Altitude'], path_data['Stay'],
            path_data['Power']))
