...
import numpy as np
import random
from global_var_150 import initial_data_p, initial_data_ch, data_p, data_ch, file_path_p, file_path_ch
...
class way():
    ...
    def info_way(self, node_current, x_current, y_current, alti_current, node_next):
        ...
        if node_current in range(self.n_ch):
            ...
            if x_current != next_x:
                match = data_ch[(data_ch["Latitude"] == x_current) & (data_ch["Longitude"] == y_current) & (
                            data_ch["Elevation"] == alti_current)]
                if not match.empty:
